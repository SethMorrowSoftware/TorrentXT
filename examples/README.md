...
| `newquickshare.livecodescript` | The **same** Quick Share, re-skinned + a better web server: a wider, shorter, **720p-friendly** two-column dashboard (plain-English 3-way "how to share", numbered steps, Receive / Transfers / Activity on the right). The right-hand list is now an actionable **Transfers & shares** surface: run **many code shares at once**, select any row to **Copy** its code again or **Stop** it (stop sharing a file, or cancel a download), each with a plain-English confirm. Plus HTTP serving polish - an O(log n) file-size probe (no more UI freeze / double read before a download), real filenames on save (`Content-Disposition`, `?dl` forces a download), `nosniff` + `Cache-Control` headers, and alphabetised directory listings. | Only for the optional passphrase lock |
